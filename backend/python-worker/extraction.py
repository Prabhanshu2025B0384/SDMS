...
class GeminiMetadataExtractor(MetadataExtractor):
    def extract(self, raw_text: str, document_type: str = "Document") -> dict[str, Any]:
        ...
        try:
            ...
            - Provide your response ONLY as a JSON object, where keys are field names and values are the extracted strings. Do not use markdown blocks, just raw JSON.
            ...
            if "candidates" in result_data and len(result_data["candidates"]) > 0:
                ...
                structured_dict = json.loads(text_response)
                print(f"Gemini returned {len(structured_dict)} fields")
                
                # Limit to 15 fields max just to be safe
                final_dict = {}
                for i, (k, v) in enumerate(structured_dict.items()):
                    if i >= 15: break
                    final_dict[k] = v
                return final_dict
            else:
